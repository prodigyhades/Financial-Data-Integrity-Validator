...
def stock_data():
    ...
    try:
        ...
        data = client.get_daily('IBM') #get_daily returns a dataframe with columns 'open', 'high', 'low', 'close', 'volume'
        return data
...
def test_column_data_types_are_correct(stock_data, column_name):
    """
    Checks that a given column's data type (dtype) is numeric.
    This test is run once for each column in NUMERIC_COLUMNS.
    """
    assert pd.api.types.is_numeric_dtype(stock_data[column_name]), \
        f"Column '{column_name}' is not numeric."

'''
- Pytest treats each parameter as a separate test. If the assertion fails for 'open', the tests for 'high', 'low', etc., 
will still run.

- The test report will pinpoint exactly which column failed (e.g., FAILED test_column_data_types_are_correct[open]).
'''